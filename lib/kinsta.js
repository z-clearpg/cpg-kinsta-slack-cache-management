...
/**
 * Find a single live environment that *exactly* matches the given query
 * (a domain name or the site title), case-insensitively. Exact — not substring —
 * so a partial or fuzzy argument never silently clears the wrong site.
 * Returns the matching environment, or null if there is no unambiguous match.
 */
async function findLiveEnvironmentByExactMatch(query) {
  ...
  const matches = environments.filter(env => {
    ...
    const domains = env.domains.map(d => d.toLowerCase());
    return siteName === needle || domains.includes(needle);
  });
 ...
 * Resolves { success: true } on 200, { success: false, message } on failure
 * or timeout. Never throws — polling errors are folded into the result.
 */
async function pollOperation(operationId, {
  ...
  return { success: false, message: 'Timed out waiting for cache clear to finish.' };
}
