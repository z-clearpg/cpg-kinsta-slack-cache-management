...
async function runCacheClear(respond, target) {
  ...
  await respond({
    ...
    text: `⌛ Clearing cache for *${target.label}*…`
  });
  ...
  try {
    ...
    if (result.success) {
      ...
      await respond({
        response_type: 'ephemeral',
        replace_original: true,
        text: `✅ Cache clear submitted for *${target.label}* — finishing up now.`
      });
    } else {
...
function registerHandlers(app) {
  ...
  // Confirmed via the picker: clear and report the confirmed result.
